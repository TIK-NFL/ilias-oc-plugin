	...
	*/
	function ilMatterhornSendfile()
	{
		...
		if(0 == strcmp(substr($uri["path"],0,strpos($_SERVER["PHP_SELF"],"/sendfile.php"))."/episode.json", $uri["path"])){
			...
			if (!preg_match('/^[0-9]+\/[A-Za-z0-9]{8}-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}-[A-Za-z0-9]{12}/', $this->params['id'])) {
				$this->errorcode = 404;
...
#		{
				header('x-sendfile: /var/www/localhost/mh/engage-player/' . substr($this->subpath, strlen($this->obj_id)));
#				header("Content-Type: application/octet-stream");
				include_once("./Services/Utilities/classes/class.ilMimeTypeUtil.php");
				$mime = ilMimeTypeUtil::getMimeType('/var/www/localhost/mh/engage-player/' . substr($this->subpath, strlen($this->obj_id)));
